...
public:
	chessPiece(int lx,int ly,pieceColor c=pieceColor::NONE,sf::Vector2f v=sf::Vector2f(0,0),pieceName p=pieceName::NONE):
	bx(lx),
	by(ly),
	color(c),
	textCoords(v),piece(p){};
	chessPiece();
	virtual ~chessPiece();
	int bx;
	int by;
...
public:
	noPiece(int x,int y):chessPiece(x,y,pieceColor::NONE,sf::Vector2f(600,0),pieceName::NONE){};
	noPiece(){};
