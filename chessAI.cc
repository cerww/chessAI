...
boardNode makeNode(const chessB& b){
	...
	chessAI::dp[re]=chessAI::dp.size();
	chessAI::dp2[chessAI::dp2.size()]=re;
	return re;
}
...
void getFullTree(const boardNode& c){
	std::array<std::unordered_map<boardNode,char>,2> levels;
	char level = 0;
	std::vector<boardNode> que(1,c);
	levels[0][c]=0;
	while(que.size()&& level<(TURNSY+1)){
		std::vector<boardNode> next;
		for(auto& current:que){
			...
			char i = level%2;
				for(auto& x:current.moveSpots[i]){
					...
					if(levels[i].find(newNode)==levels[i].end()){
						levels[i][newNode] = level;
		...
		for(auto& m:que){
			...
			int c = level%2;
				...
				for(unsigned i = 0;i<current->moveSpots[c].size();++i){
					...
					if(levels[c].find(chessAI::dp[newNode])==levels[c].end()){
						levels[c][chessAI::dp[newNode]] = level;
						next.push_back(chessAI::dp[newNode]);
					}
				}
			//}
